...
export default function ServiceSaleForm({ onCreated }) {
  ...
  async function handleSubmit(event) {
    ...
    try {
      const response = await fetch("/api/v1/user/services/create", {
        method: "POST",
        headers: {
      ...
      if (!response.ok) {
        throw new Error(result.error || "Could not publish this service")
      }
      ...
      setMessage({
        type: "success",
        text: "Your service is now listed on the platform.",
      })
        ...
        <div className="relative min-h-56 overflow-hidden bg-[#214a38] p-6 text-white sm:p-8">
          ...
          <div className="relative">
            ...
            <h2 className="mt-2 max-w-xs text-3xl font-semibold leading-tight tracking-tighter">
              Offer your service to growers nearby.
            </h2>

            <p className="mt-3 max-w-sm text-sm leading-6 text-[#d2e0cf]">
              List ploughing, irrigation, pest control, or any farm service so
              buyers can book you directly.
            </p>
          </div>
        </div>
              ...
              <div className="mt-2 flex items-center gap-4 rounded-lg border border-dashed border-[#c5d2c2] bg-[#fbfcf9] p-4">
                ...
                <div className="min-w-0">
                  <p className="text-xs font-normal text-[#718078]">
                    Upload a photo of your service or equipment.
                  </p>
                  <input
                    accept="image/*"
