...
export default function CropSaleForm() {
  ...
  async function handleSubmit(event) {
    ...
    try {
      const response = await fetch("/api/v1/user/crops/create", {
        method: "POST",
      ...
      if (!response.ok) {
        throw new Error(result.error || "Could not publish this crop");
      }
        ...
        <div className="relative min-h-56 overflow-hidden bg-[#214a38] p-6 text-white sm:p-8">
          ...
          <div className="relative">
            ...
            <p className="mt-3 max-w-sm text-sm leading-6 text-[#d2e0cf]">
              Add clear details so buyers can discover and plan around your
              crop.
            </p>
          </div>
        </div>
        <form onSubmit={handleSubmit} className="p-6 sm:p-8">
          ...
          <div className="mt-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            {message.text ? (
              <p
                role="status"
                className={`flex items-center gap-2 text-sm ${message.type === "success" ? "text-[#5b823e]" : "text-[#a14f3e]"}`}
              >
              ...
              <p className="text-xs text-[#92a095]">
                All amounts should be listed in metric units.
              </p>
            )}
            <button
